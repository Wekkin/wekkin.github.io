...
# -*- coding: utf-8 -*-
"""
UNH股票未来2年走势预测分析工具 - 改进版
UnitedHealth Group (UNH) Stock Price Prediction for Next 2 Years - Improved Version
"""
...
class UNHStockPredictor:
    ...
    def predict_future_prices(self, days=730):  # 2年 = 730天
        ...
        # 限制预测值在合理范围内 (当前价格的0.1-10倍)
        linear_pred = np.clip(linear_pred, current_price * 0.1, current_price * 10)
        ...
        for i in range(days):
            if i == 0:
                ...
                new_price = exp_predictions[-1] * random_factor
                exp_predictions.append(new_price)
        predictions['Exponential'] = exp_predictions
        
        # 方法3: 移动平均趋势 (改进版)
        # 使用更短期的趋势，避免极端值
        recent_ma = self.data['MA50'].dropna()
        if len(recent_ma) >= 20:
            ma_trend = (recent_ma.iloc[-1] - recent_ma.iloc[-20]) / 20
        else:
            ma_trend = 0
        ...
        for i in range(days):
            # 限制趋势影响，避免极端值
            trend_factor = min(abs(ma_trend), current_price * 0.01)  # 最大1%的日变化
            if ma_trend > 0:
                new_price = current_price + (trend_factor * (i + 1))
            else:
                new_price = current_price - (trend_factor * (i + 1))
            ma_predictions.append(max(new_price, current_price * 0.5))  # 最低不低于当前价格的50%
        ...
        # 限制波动率，避免极端值
        std_return = min(std_return, 0.05)  # 最大5%的日波动率
        ...
        for _ in range(100):  # 100次模拟
            ...
            for _ in range(days):
                ...
                # 限制单日变化幅度
                daily_return = np.clip(daily_return, -0.1, 0.1)  # 最大±10%的日变化
        ...
        for i in range(days):
            # 基于年化收益率和波动率的预测
            expected_price = current_price * (1 + annual_return/252) ** (i + 1)
            # 添加随机波动
            random_vol = np.random.normal(0, volatility)
            final_price = expected_price * (1 + random_vol)
            vol_predictions.append(max(final_price, current_price * 0.3))
        
        predictions['Volatility_Based'] = vol_predictions
    ...
    def generate_forecast_report(self):
        ...
        for method, pred in self.predictions.items():
            ...
            # 只保留合理的预测结果
            if final_price > 0 and final_price < current_price * 20:
                valid_predictions[method] = pred
                change_pct = ((final_price - current_price) / current_price) * 100
                print(f"{method:20}: ${final_price:.2f} ({change_pct:+.2f}%)")
                
                if confidence_interval is not None:
                    print(f"{'':20}  置信区间: ${confidence_interval[0]:.2f} - ${confidence_interval[1]:.2f}")
        ...
        if valid_predictions:
            ...
            for method, pred in valid_predictions.items():
                if method == 'Monte_Carlo':
                    mean_pred = np.mean(pred, axis=0)
                    final_prices.append(mean_pred[-1])
                else:
                    final_prices.append(pred[-1])
            ...
            for i, (method, pred) in enumerate(self.predictions.items()):
                if method == 'Monte_Carlo':
                    ...
                    ax5.plot(future_dates[:len(mean_pred)], mean_pred, 
                            label=f'{method}预测', color=colors[i % len(colors)], linestyle='--')
                else:
